...
if __name__ == "__main__":
    run_training_experiment()def run_epoch(data_iter, model: Transformer, loss_fn: nn.Module, optimizer: Optional[torch.optim.Optimizer], scheduler=None, epoch_num: int = 0, is_train: bool = True, device: str = "cpu") -> float:
    ...
    print(f"Epoch {epoch_num:03d} " f"[{mode_str}] " f"loss={avg_loss:.4f}")
...
def evaluate_bleu(model: Transformer, test_dataloader: DataLoader, tgt_vocab, device: str = "cpu", max_len: int = 100) -> float:
    ...
    with torch.no_grad():
        for src, tgt in test_dataloader:
            ...
            for i in range(src.size(0)):
                ...
                pred_ids = greedy_decode(model, src_i, src_mask, max_len=max_len, start_symbol=SOS_IDX, end_symbol=EOS_IDX, device=device).squeeze(0).tolist()
                ...
                hyp = " ".join(ids_to_tokens(pred_ids))

                ref = " ".join(ids_to_tokens(tgt[i].tolist()))
    ...
    print(f"Test BLEU: " f"{bleu_score:.2f}")
...
def save_checkpoint(model: Transformer, optimizer: torch.optim.Optimizer, scheduler, epoch: int, path: str = "checkpoint.pt") -> None:
    ...
    print(f"Checkpoint saved → " f"{path} " f"(epoch {epoch})")
...
def load_checkpoint(path: str, model: Transformer, optimizer: Optional[torch.optim.Optimizer] = None, scheduler=None) -> int:
    ...
    print(f"Checkpoint loaded ← " f"{path} " f"(epoch {epoch})")
...
def run_training_experiment() -> None:
    ...
    print(f"Using device: " f"{device}")

    train_loader, val_loader, test_loader, src_vocab, tgt_vocab = get_dataloaders(batch_size=cfg.batch_size, min_freq=cfg.min_freq)

    print(f"Src vocab size: " f"{len(src_vocab)} | " f"Tgt vocab size: " f"{len(tgt_vocab)}")

    model = Transformer(src_vocab_size=len(src_vocab), tgt_vocab_size=len(tgt_vocab), d_model=cfg.d_model, N=cfg.N, num_heads=cfg.num_heads, d_ff=cfg.d_ff, dropout=cfg.dropout).to(device)
    ...
    print(f"Trainable parameters: " f"{n_params:,}")
    ...
    loss_fn = LabelSmoothingLoss(vocab_size=len(tgt_vocab), pad_idx=PAD_IDX, smoothing=cfg.smoothing)
    ...
    for epoch in range(cfg.num_epochs):
        ...
        save_checkpoint(model, optimizer, scheduler, epoch, path="checkpoint_latest.pt")
