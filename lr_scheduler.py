...
class NoamScheduler(LRScheduler):
    ...
    Applies a warm-up phase where LR increases linearly, followed by a decay
    phase where LR decreases proportional to the inverse square root of the
    step number.
    ...
    def _get_lr_scale(self) -> float:
        ...
        step  = self.last_epoch + 1   (avoids step=0)
        scale = d_model^(-0.5) * min(step^(-0.5), step * warmup_steps^(-1.5))
        """
        step = self.last_epoch + 1          # 1-indexed step count
